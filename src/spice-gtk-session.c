...
static void drag_begin(SpiceGtkSession *self, GStrv target_names);
...
static gboolean filter_target(const gchar *name)
{
     ...
     */
    /* TODO: should *_TARGETS be filtered too? */
    ...
    for (i = 0; exclude[i]; i++)
        if (!g_ascii_strcasecmp(name, exclude[i]))
            return TRUE;
    return FALSE;
}
...
static void clipboard_data_retrieved(RunInfo *ri, GdkAtom type, gboolean text,
                                     ...
                                     gpointer user_data)
{
    ...
    clipboard_data_retrieved(ri,
        gdk_atom_intern_static_string(atom2agent[ri->info].xatom),
        atom2agent[ri->info].vdagent == VD_AGENT_CLIPBOARD_UTF8_TEXT,
        8, data, size);
}
...
static void selection_grab(SpiceMainChannel *main, guint selection,
                           GStrv targets, gpointer user_data)
{
    g_return_if_fail(SPICE_IS_GTK_SESSION(user_data));
    ...
    clear_clipboard_targets(s, selection);
    s->nclip_targets[selection] = g_strv_length(targets);
    s->clip_targets[selection] = g_new(GtkTargetEntry, s->nclip_targets[selection]);
    g_message("%s:: sel=%u", __func__, selection);
    for (i = 0; i < s->nclip_targets[selection]; i++) {
        s->clip_targets[selection][i].target = g_strdup(targets[i]);
        s->clip_targets[selection][i].info = 0;
        g_message("    %s", targets[i]);
    }
    ...
    if (!gtk_clipboard_set_with_owner(cb, s->clip_targets[selection], s->nclip_targets[selection],
                                      clipboard_get, clipboard_clear, G_OBJECT(self))) {
        g_warning("clipboard grab failed");
        return;
    }
                                       ...
                                       gpointer user_data)
{
    ...
    if (selection == -1) {
        g_critical("%s: Invalid selection", __func__);
        goto cleanup;
    }
    ...
    if (SPICE_IS_GTK_SESSION(self) == FALSE) {
        g_critical("%s: Not a SpiceGtkSession", __func__);
        goto cleanup;
    }
                                  ...
                                  gpointer user_data)
{
    ...
    if (spice_main_channel_agent_test_capability(s->main, VD_AGENT_CAP_SELECTION_DATA)) {
        ...
        spice_main_channel_selection_send_data(s->main, selection, m, name, data, len);
    } else {
        for (m = 0; m < SPICE_N_ELEMENTS(atom2agent); m++) {
            if (strcasecmp(name, atom2agent[m].xatom) == 0) {
                break;
                                  ...
                                  guint type, gpointer user_data)
{
    ...
    if (type == VD_AGENT_CLIPBOARD_UTF8_TEXT) {
        TextRequestInfo *info = g_new(TextRequestInfo, 1);
        info->session_ref = get_weak_ref(self);
        gtk_clipboard_request_text(cb, clipboard_received_text_cb, info);
    } else {
        for (m = 0; m < SPICE_N_ELEMENTS(atom2agent); m++) {
...
static void selection_request(SpiceMainChannel *main, guint selection,
                              const gchar *target, gpointer user_data)
{
    g_message("%s:: sel=%u, target=%s", __func__, selection, target);
    ...
    GtkClipboard *cb = get_clipboard_from_selection(s, selection);
    g_return_if_fail(cb != NULL);
                            ...
                            gpointer user_data)
{
    ...
    if (SPICE_IS_INPUTS_CHANNEL(channel) && SPICE_INPUTS_CHANNEL(channel) == s->inputs) {
        s->inputs = NULL;
    }
}
...
/* ---------------------------------------------------------------- */
/* private functions (drag&drop related)                            */
...
{
    g_return_if_fail(SPICE_IS_GTK_SESSION(user_data));
    selection_data_get(user_data, VD_AGENT_DND_SELECTION, selection_data, 0);
}
                     ...
                     gpointer        user_data)
{
    ...
    g_return_if_fail(s->drag == drag_context); // really?
    s->drag = NULL;
    gtk_widget_destroy(source);

    if (s->inputs)
        // spice_inputs_key_press_and_release(s->inputs, 0x01);
        spice_inputs_channel_button_release(s->inputs,
            button_gdk_to_spice(s->drag_event->button.button),
            button_mask_gdk_to_spice(s->drag_event->button.state));
...
static void drag_begin(SpiceGtkSession *self, GStrv target_names)
{
    ...
    g_return_if_fail(s->drag_event != NULL);
    if (s->drag != NULL)
        return;
    ...
    g_signal_connect(source, "drag-failed", G_CALLBACK(drag_failed), self);
    g_signal_connect(source, "drag-end", G_CALLBACK(drag_end), self);

    targets = gtk_target_list_new(NULL, 0);
    for (i = 0; target_names[i]; i++)
        gtk_target_list_add(targets, gdk_atom_intern(target_names[i], FALSE), 0, i);

    s->drag = gtk_drag_begin_with_coordinates(source, targets, GDK_ACTION_COPY,
                                              s->drag_event->button.button,
                                              s->drag_event,
                                              (gint)s->drag_event->button.x,
                                              (gint)s->drag_event->button.y);
...
{
    g_return_val_if_fail(SPICE_IS_GTK_SESSION(self), NULL);
...
{
    g_return_val_if_fail(SPICE_IS_GTK_SESSION(self), 0);

    SpiceGtkSessionPrivate *s = self->priv;
    if (s->drag_event)
        return s->drag_event->button.state;
    else
...
{
    SpiceGtkSession *self = SPICE_DISPLAY(widget)->priv->gtk_session;
    SpiceGtkSessionPrivate *s = self->priv;
...
// G_GNUC_BEGIN_IGNORE_DEPRECATIONS
    ...
    if (GDK_IS_X11_DISPLAY(gdk_display)) {
        /* break dnd grab to cancel */
        GtkWidget *invisible = gtk_window_new(GTK_WINDOW_POPUP);
        GdkDevice *device = gdk_drag_context_get_device(s->drag);
    ...
    {
        gtk_drag_cancel(s->drag);
        return TRUE;
    }
#else
    {
        g_warning("Drag cannot be cancelled, compile with GTK+ 3.16.0 or higher.");
        return FALSE;
    }
#endif
// G_GNUC_END_IGNORE_DEPRECATIONS
...
void spice_gtk_session_button_event(SpiceGtkSession *self, GdkEventButton *event)
{
    ...
    switch (event->type) {
        case GDK_BUTTON_PRESS:
            if (s->drag_event)
                break;
            s->drag_event = gdk_event_copy((GdkEvent *)event);
            /* FIXME: what button state should we pass to motion_event() ? */
            switch (event->button) {
                case GDK_BUTTON_PRIMARY:
                    s->drag_event->button.state |= GDK_BUTTON1_MASK;
                    break;
                case GDK_BUTTON_MIDDLE:
                    s->drag_event->button.state |= GDK_BUTTON2_MASK;
                    break;
                case GDK_BUTTON_SECONDARY:
                    s->drag_event->button.state |= GDK_BUTTON3_MASK;
                    break;
            }
            break;
        case GDK_BUTTON_RELEASE:
            /* drag_event is used and freed in drag_end() */
            if (s->drag || !s->drag_event || event->button != s->drag_event->button.button)
                break;
            gdk_event_free(s->drag_event);
            s->drag_event = NULL;
            break;
        default:
            break;
